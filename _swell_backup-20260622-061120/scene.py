            ...
            if o.kind is TaikoType.DRUMROLL:
                ...
                if self.sk_roll:
                    ...
                    while x <= tail + 1:
                        ...
                        x += step
            else:  # SWELL: scroll in, then SIT at the target until end; mash to fill
                if t > end_ms:
                    continue
                if t < o.time_ms:
                    ...
                    sx = g.target_x
                prog = 0.0
                if end_ms > o.time_ms and t >= o.time_ms:
                    prog = min(1.0, (t - o.time_ms) / (end_ms - o.time_ms))
                ring_d = g.pf_h * 0.95
                # expanding pulse (additive-ish via bright ring) while active
                if t >= o.time_ms:
                    puls = ((t - o.time_ms) % 220) / 220.0
                    sp.append(Sprite(sx, cy, ring_d * (0.5 + 0.55 * puls),
                                     ring_d * (0.5 + 0.55 * puls), "argon_swell_ring",
                                     (1, 1, 1, 0.35 * (1 - puls))))
                sp.append(Sprite(sx, cy, ring_d, ring_d, "argon_swell_ring",
                                 (1, 1, 1, 0.45 + 0.5 * prog)))
                sp.append(Sprite(sx, cy, g.big_d, g.big_d, "argon_swell",
                                 (1, 1, 1, 1)))
